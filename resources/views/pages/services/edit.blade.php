...
<div class="container py-5">
  <h1 class="fw-bold text-primary-blue mb-4">Edit Service</h1>

  <form action="{{ route('services.update', $service->slug) }}" method="POST" class="p-4 bg-light rounded shadow-sm">
    @csrf @method('PUT')
    ...
    <div class="row">
      ...
      <div class="col-md-6 mb-3">
        ...
        <input type="text" id="slug" name="slug" class="form-control" value="{{ old('slug', $service->slug) }}" required>
    ...
    <div class="mb-3">
      ...
      <textarea name="body" rows="4" class="form-control">{{ old('body', $service->body) }}</textarea>
    ...
    <div class="row">
      <div class="col-md-6 mb-3">
        ...
        <input type="text" name="hero_heading" class="form-control" value="{{ old('hero_heading', $service->hero_heading) }}">
        @error('hero_heading') <div class="text-danger small">{{ $message }}</div> @enderror
      </div>
      <div class="col-md-6 mb-3">
        <label class="form-label fw-bold">Hero Subheading</label>
        <input type="text" name="hero_subheading" class="form-control" value="{{ old('hero_subheading', $service->hero_subheading) }}">
    ...
    <div class="mb-3" id="features-wrapper">
      ...
      @forelse(old('features', $features) as $feature)
    ...
    {{-- Process Steps --}}
    @php 
      $steps = is_string($service->process_steps) 
        ? json_decode($service->process_steps, true) 
    ...
    <div class="mb-3" id="steps-wrapper">
      ...
      @forelse(old('process_steps', $steps) as $step)
        <div class="input-group mb-2">
    ...
    {{-- Partners --}}
    @php 
      $partners = is_string($service->partners) 
        ? json_decode($service->partners, true) 
        : ($service->partners ?? []); 
    @endphp
    <div class="mb-3" id="partners-wrapper">
      ...
      @forelse(old('partners', $partners) as $partner)
        <div class="input-group mb-2">
    ...
    <div class="d-flex justify-content-end gap-2">
      <button type="submit" class="btn bg-accent-orange text-white">Update Service</button>
